...
namespace techretail_api.Services
{
    ...
    public class ProductsService : IProductsService
    {
        ...
        public async Task<PagedResult<Product>> GetProductsAsync(string? search = null, bool lowStockOnly = false, int page = 1, int pageSize = 100, bool maskCostPrice = false, bool maskSellingPrice = false)
        {
            ...
            if (lowStockOnly)
            {
                // Use each product's own MinStockQuantity threshold, not a global magic number
        ...
        {
            if (product.SKU != null && product.SKU.StartsWith("TEST-", StringComparison.OrdinalIgnoreCase))
            {
                throw new ArgumentException("Không được phép tạo sản phẩm test (SKU bắt đầu bằng TEST-) trên môi trường Production.");
            }
