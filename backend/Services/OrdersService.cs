...
namespace techretail_api.Services
{
    ...
    {
        Task<PagedResult<Order>> GetOrdersAsync(int page = 1, int pageSize = 50, string? status = null, string? search = null, bool maskFinancialData = false, bool isSalesStaff = false, Guid? currentUserId = null, DateTime? fromDate = null, DateTime? toDate = null, string? createdBy = null);
    ...
    public class OrdersService : IOrdersService
    {
        ...
        public async Task<PagedResult<Order>> GetOrdersAsync(int page = 1, int pageSize = 50, string? status = null, string? search = null, bool maskFinancialData = false, bool isSalesStaff = false, Guid? currentUserId = null, DateTime? fromDate = null, DateTime? toDate = null, string? createdBy = null)
        {
            ...
            if (isSalesStaff && currentUserId.HasValue)
            {
                query = query.Where(o => o.CreatedBy == currentUserId.Value);
            }
        ...
        public async Task<Order> CreateOrderAsync(Order order)
        {
            ...
            // Generate collision-safe OrderCode: ORD-YYYYMMDD-XXXXXXXX (date + 8 random hex chars)
            ...
            try
            {
                ...
                _context.SystemLogs.Add(new SystemLog
                {
                    ...
                    TableName = "Orders",
                    OldValues = null,
                    NewValues = $"Tạo mới đơn hàng {order.OrderCode}",
                    SeverityLevel = "Info",
            ...
            var creator = await _context.Users.FindAsync(order.CreatedBy);
            await _hub.Clients.Groups("Admin", "Manager", "Sales", "Warehouse")
                .SendAsync("ReceiveActivity", new
                {
                    ...
                    type = "info",
                    title = "CreateOrder",
                    description = $"{creator?.FullName ?? "Hệ thống"} đã thao tác tạo mới trên Orders.",
                    createdAt = DateTime.UtcNow,
                    badgeText = "Thông tin"
                });
        ...
        public async Task UpdateOrderStatusAsync(Guid orderId, string newStatus, string? reason, bool isAdmin, Guid currentUserId = default)
        {
            ...
            try
            {
                ...
                {
                    // Batch load all products in 1 query instead of N FindAsync calls
                ...
                if (!isValidForward && isAdmin)
                {
                    _context.SystemLogs.Add(new SystemLog
                    {
                        ...
                        UserId = currentUserId, // Log the actual user who performed the override
                        TableName = "Orders",
                        OldValues = oldStatus,
        ...
        public async Task DeleteOrderAsync(Guid orderId)
        {
            ...
            try
            {
                ...
                // Batch load products to restore stock in 1 query
                if (order.OrderStatus == "Confirmed" || order.OrderStatus == "Shipped" || order.OrderStatus == "Delivered")
                {
                    var productIds = order.OrderDetails.Select(d => d.ProductId).ToList();
                    var products = await _context.Products
