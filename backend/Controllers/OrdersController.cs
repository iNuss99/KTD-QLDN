...
namespace techretail_api.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
            ...
            [FromQuery] string? createdBy = null)
        {
            ...
            if (Guid.TryParse(currentUserIdStr, out var parsedId))
            {
                currentUserId = parsedId;
            }

            var orders = await _ordersService.GetOrdersAsync(page, pageSize, status, search, maskFinancialData, isSalesStaff, currentUserId, fromDate, toDate, createdBy);
            return Ok(orders);
        }
