...
import {
  ...
  useRecentActivities, fetchMarginDetails
...
export default function DashboardView({
  ...
  const { data: kpis = { totalRevenue: 0, totalCogs: 0, grossProfit: 0, margin: 0, revenueChange: 0 } } = useDashboardKpis(dateRange);
  const { data: revenueDataRaw = [] } = useRevenueChart(dateRange);
  const { data: topProducts = [] } = useTopProducts(dateRange);
  const { data: orderDistribution = null } = useOrderDistribution(dateRange);
  const { data: salesTrend = [] } = useSalesTrend(dateRange);
  const { data: marginRaw, isLoading: chartsLoading } = useMarginDetails(marginPage, marginPageSize, dateRange, canViewFinancials);
  ...
  const { data: recentActivitiesRaw = [] } = useRecentActivities();
  const recentActivities = recentActivitiesRaw as any[];
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      const allData = await fetchMarginDetails(1, 10000, dateRange);
      const items = allData.items || [];
      const BOM = "\uFEFF";
      const csvContent = BOM + "Mã đơn hàng,Khách hàng,Ngày tạo,Doanh thu,Giá vốn,Lợi nhuận,Margin (%)\n"
        + items.map((e: any) => `${e.orderCode},${e.customerName},${new Date(e.createdAt).toLocaleDateString('vi-VN')},${e.revenue},${e.cost},${e.profit},${e.margin}%`).join("\n");
        
      const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.setAttribute("href", url);
      link.setAttribute("download", `bang_margin_${dateRange}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (error) {
      console.error(error);
      onShowNotification('Lỗi khi xuất báo cáo!');
    }
  };
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[10px] text-slate-400 block mt-1 font-medium">
                      {act.time || new Date(act.createdAt).toLocaleString('vi-VN')}
                    </span>
                  </div>
                </div>
              ))
                  ...
                  marginData.map((order, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-6 py-3 text-right text-rose-600">
                        {order.cost === 0 ? (
                          <div className="flex items-center justify-end gap-1" title="Đơn hàng thiếu chi tiết giá vốn">
                            <AlertTriangle size={12} className="text-amber-500" />
                            <span>{formatVND(order.cost)}</span>
                          </div>
                        ) : (
                      ...
                      <td className="px-6 py-3 text-right">
                        {order.cost === 0 ? (
                          <span className="px-2 py-1 rounded text-[10px] font-bold bg-amber-50 text-amber-600 border border-amber-200" title="Cảnh báo: Lợi nhuận tuyệt đối do thiếu COGS">
                            ⚠️ 100%
                          </span>
                        ) : (
