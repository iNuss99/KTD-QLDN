        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col gap-4">
          ...
          <div className="p-3.5 bg-slate-50 border border-slate-200/50 rounded-xl">
            ...
            <div className="flex items-baseline justify-between mt-1">
              ...
              <span className={`text-[10px] font-bold rounded px-1.5 py-0.5 flex items-center gap-0.5 ${(metrics?.revenueChange ?? 0) >= 0 ? 'text-emerald-700 bg-emerald-50 border border-emerald-100' : 'text-rose-700 bg-rose-50 border border-rose-100'}`}>
                {(metrics?.revenueChange ?? 0) >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
                {metrics ? Math.abs(metrics.revenueChange ?? 0) : 0}%
              </span>
            </div>
          </div>
