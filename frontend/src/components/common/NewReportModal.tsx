...
import { generateReport } from '../../utils/exportReport';
...
export default function NewReportModal({ onClose, onShowNotification }: NewReportModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Actually generate and download the report
      await generateReport(reportType, quarter, format, includeAI);
      ...
      setTimeout(() => {
        ...
        onShowNotification(`Thành công: Báo cáo ${reportType.toUpperCase()} cho ${quarter} đã được tạo và tải xuống.`);
        onClose();
      }, 500);
    } catch (error) {
      console.error(error);
      setIsGenerating(false);
      onShowNotification('Lỗi: Không thể tạo báo cáo. Vui lòng thử lại sau.');
    }
  };
